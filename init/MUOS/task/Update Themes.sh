...
if [ ! -d "${DC_STO_ROM_MOUNT}/MUOS/theme" ]; then
...
WIFI_CONNECTED=$(ping -c 1 github.com | grep "bytes from" | wc -l)
...
THEME_FILES="$THEME_FILES \"$FILE\""
done <"${DC_STO_ROM_MOUNT}/MUOS/theme"
...
for FILE in "$THEME_FILES"; do
  ...
  /bin/sh "${DC_STO_ROM_MOUNT}/MUOS/script/mux/extract.sh" "/tmp/${FILE}"
